...
const MessDetail = () => {
  ...
  const fetchMessPlanDetails = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`/api/mess/plans/${id}`);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="bg-white rounded-lg shadow p-6 mb-6">
              <h1 className="text-3xl font-bold text-gray-900 mb-4">{plan.planName}</h1>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className="text-center">
                  ...
                  <p className="font-semibold">{plan.planType}</p>
                </div>
                <div className="text-center">
                  <FaUsers className="h-6 w-6 text-green-600 mx-auto mb-2" />
                  <p className="text-sm text-gray-600">Capacity</p>
                  <p className="font-semibold">{plan.currentSubscribers}/{plan.capacity}</p>
                </div>
                <div className="text-center">
                ...
                <div className="text-center">
                  ...
                  <p className="font-semibold">{plan.hygieneRating}/5</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {plan.menu && Object.keys(plan.menu).map((meal) => (
                  <div key={meal} className="border border-gray-200 rounded-lg p-4">
                    <h3 className="font-semibold text-gray-900 mb-3 capitalize">{meal}</h3>
            ...
            {/* Meal Timings */}
            <div className="bg-white rounded-lg shadow p-6 mb-6">
              <h2 className="text-xl font-semibold text-gray-900 mb-4">Meal Timings</h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {plan.mealTimings && Object.keys(plan.mealTimings).map((meal) => (
                  <div key={meal} className="text-center">
                    ...
                    <p className="text-gray-600">{plan.mealTimings[meal]}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow p-6 sticky top-8">
              ...
              <div className="space-y-4 mb-6">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold">{plan.capacity - plan.currentSubscribers}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Meal Types</span>
                  <span className="font-semibold">{plan.mealTypes?.length || 0}</span>
                </div>
              </div>

              <button className="w-full bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-4 rounded-lg transition-colors mb-4">
                Subscribe Now
              </button>
              ...
              <div className="mt-6 pt-6 border-t border-gray-200">
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-3">
                    <p className="font-medium text-gray-900">{plan.provider?.name}</p>
                    <p className="text-sm text-gray-600">{plan.provider?.phone}</p>
                  </div>
                </div>
              </div>
            </div>
